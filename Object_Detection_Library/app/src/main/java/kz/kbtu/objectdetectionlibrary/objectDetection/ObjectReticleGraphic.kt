...
import kz.kbtu.objectdetectionlibrary.camera.CameraReticleAnimator
import kz.kbtu.objectdetectionlibrary.camera.GraphicOverlay
...
internal class ObjectReticleGraphic(
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (isSuccess) {
            ripplePaint.color = ContextCompat.getColor(context, R.color.green)
            ripplePaint.strokeWidth = rippleStrokeWidth.toFloat()
            canvas.drawRoundRect(
                RectF(
                    ...
                    boxRect.right,
                    boxRect.bottom
                ), boxCornerRadius, boxCornerRadius, ripplePaint
            )
        } else {
